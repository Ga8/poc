...
function extract() {
    ...
    let pathExtract = urlParams.get("pathExtract")?? '/addresses';
    let extract = urlParams.get("extract") ?? "addresslist";

    var ifrmExtract = document.createElement("iframe");
    ifrmExtract.setAttribute("src", pathExtract);
    ...
    ifrmExtract.onload = function () {
        setTimeout(function () {
            ...
            alert(contentWindow.document.getElementsByClassName(extract)[0]?.outerHTML);
        }, 2000);
    };
}
